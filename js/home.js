...
document.addEventListener("DOMContentLoaded", function () {
  ...
  .then(data => {
    if (data.isSuccess) {
      ...
      // 연속 기록
      const userContinuousRecordDayElement = document.getElementById("user-continuousRecordDay");
      userContinuousRecordDayElement.textContent = `연속 ${continuousRecordDay}일째 기록 완료`;

      // 술 없는 날 
      const alcoholFreeDayElement = document.getElementById("user-alcoholFreeDay");
    ...
    function updateFoodCount(food, count) {
      const foodContainer = document.querySelector(`.food-container[data-food="${food}"]`);
  ...
  document.querySelector(`.food-container[data-food="${defaultFood}"]`).style.display = 'flex';
  ...
  dropdownMenu.addEventListener('click', function (event) {
    const selectedFood = event.target.getAttribute('data-food');
      if (selectedFood) {
        ...
        const selectedFoodContainer = document.querySelector(`.food-container[data-food="${selectedFood}"]`);
  ...
  .then(data => {
    if (data.isSuccess && data.result) {
      ...
      const monthlyDrinkCountElement = document.getElementById("user-monthly-drink-count");
      monthlyDrinkCountElement.textContent = `음주빈도 ${monthlyDrinkCount}회`;

      const monthlyDrinkCountDifferenceElement = document.getElementById("user-monthly-drink-count-difference");
